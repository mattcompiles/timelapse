...
const optionDefinitions = [
  ...
  { name: 'images', alias: 'i', type: String, defaultOption: true },
  { name: 'fps', alias: 'f', type: Number, defaultOption: true },
  {
    name: 'location',
    alias: 'l',
    type: String,
...
module.exports = argv => {
  ...
  const capturedImages = fs
    .readdirSync(images)
    .map(file => path.join(__dirname, images, file));

  videoshow(capturedImages, getVsideoOptions(fps))
